...
\begin{flushleft}
    ...
    a strict partial order is asymmetric,  transitive
    
    if we take the reflective closure of that then the relation will be asymmetric, transitive and reflective

    then we only have to prove that reflective and asymmetric implies antisymmetric 
    
    prove:\\ 
    refl $\wedge$ asym $\rightarrow$ antisym

    definition for refl means $ \forall x \in R ( \exist xRx)$
    definition for asym


    PBC ,  refl $\wedge$ asym $\wedge$ not antisym
